...
{
    public class ApiMembershipTableProvider : IMembershipTable
    {
        private readonly ISvcClientFactory _svcClientFactory;
        private readonly ILogger<ApiMembershipTableProvider> _logger;
        ...
        public ApiMembershipTableProvider(ISvcClientFactory svcClientFactory, ILogger<ApiMembershipTableProvider> logger, IConfiguration configuration)
        {
            _svcClientFactory = svcClientFactory;
            _logger = logger;
            _clusterId = configuration["clusterId"];
        }
        ...
        public async Task InitializeMembershipTable(bool tryInitTableVersion)
        {
            ...
            await WithLog(async () =>
            {
                await cl.InitializeMembershipTableAsync(new ApiRef.InitializeMembershipTableRequest { TryInitTableVersion = tryInitTableVersion, ClusterId = _clusterId });
            });
        }
